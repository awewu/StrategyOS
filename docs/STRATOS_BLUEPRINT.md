...
### 11.3 五事七计（组织战略就绪度评估 · 非打分）

> **v1.2 重定位：** 五事七计从「决策 Gate」**迁出**，改置于 **Normative 组织与文化层（`/culture`）**，作为基于战略规划的**周期性组织评价**。决策 Gate（`/gates`）仅保留 Invest / Innovate / Deliver。代码：`lib/culture/wushi.ts`。

**五事（维度归属）：**

| 古文 | 白话 | 归属 |
|------|------|------|
| 道 | 上下同欲 · 与使命/四个满意一致 | 组织原生（`/culture`） |
| 将 | 关键岗位负责人与能力 | 组织原生（`/culture`） |
| 法 | 组织与流程就绪 | 组织原生（`/culture`） |
| 天 | 时机/宏观窗口 | 外部 · 引用 `/market` |
| 地 | 市场/区域根基 | 外部 · 引用 `/compass` |

**七计：** 对标竞品七问（主孰有道…赏罚孰明）— **定性 + 证据指引**，对标数据引用 `/market`（五力 / Hermes）。

输出：**组织战略适配风险清单**，不算「胜算 XX%」。

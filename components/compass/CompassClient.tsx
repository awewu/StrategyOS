...
import type { CompassBundle, CompassMilestone, NorthStar, PremiseAudit } from "@/lib/compass/types";
...
function NorthStarEditModal({ northStar, saving, onClose, onSave }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <div className="max-h-[88vh] w-full max-w-lg overflow-y-auto rounded-xl border border-[var(--surface-border)] bg-[var(--color-bg-surface)] p-6 shadow-xl">
        <h3 className="mb-1 text-base font-semibold">{northStar ? "编辑使命愿景" : "录入使命愿景"}</h3>
        <p className="mb-4 text-xs text-[var(--color-text-muted)]">5年终极方向，路径风险与所需增速将据此自动反推。</p>
        <div className="space-y-3">
          <div>
            <label className="mb-1 block text-xs font-medium text-[var(--color-text-muted)]">使命 · 为何存在</label>
            <textarea rows={2} className={inp} value={form.mission} onChange={e => setForm(f => ({ ...f, mission: e.target.value }))} placeholder="如：让每个中国家庭和建筑用上高效、可靠的热能系统" />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-[var(--color-text-muted)]">愿景 · 终点形态</label>
            <textarea rows={2} className={inp} value={form.vision} onChange={e => setForm(f => ({ ...f, vision: e.target.value }))} placeholder="如：2030年成为中国热泵与热水领域综合竞争力第一的民营品牌集团" />
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="mb-1 block text-xs font-medium text-[var(--color-text-muted)]">目标年</label>
              <input type="number" className={inp} value={form.targetYear} onChange={e => setForm(f => ({ ...f, targetYear: +e.target.value }))} />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-[var(--color-text-muted)]">终点营收（万元）</label>
              <input type="number" className={inp} value={form.revenueTarget} onChange={e => setForm(f => ({ ...f, revenueTarget: +e.target.value }))} />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-[var(--color-text-muted)]">目标利润率 {Math.round(form.profitMarginTarget * 100)}%</label>
              <input type="range" min={0} max={50} value={Math.round(form.profitMarginTarget * 100)} onChange={e => setForm(f => ({ ...f, profitMarginTarget: +e.target.value / 100 }))} className="mt-2 w-full accent-[var(--color-accent)]" />
            </div>
          </div>
          <input className={inp} value={form.marketPositionDesc ?? ""} onChange={e => setForm(f => ({ ...f, marketPositionDesc: e.target.value || null }))} placeholder="市场地位（可选）" />
          <input className={inp} value={form.geographyDesc ?? ""} onChange={e => setForm(f => ({ ...f, geographyDesc: e.target.value || null }))} placeholder="地理覆盖（可选）" />
          <input className={inp} value={form.brandDesc ?? ""} onChange={e => setForm(f => ({ ...f, brandDesc: e.target.value || null }))} placeholder="品牌格局（可选）" />
        </div>

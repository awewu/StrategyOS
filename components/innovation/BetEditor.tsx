        ...
        <div className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-[1fr_90px_150px]">
            <div>
              ...
              <Input fullWidth inputSize="sm" value={title} onChange={(e) => setTitle(e.target.value)} />
            </div>
            <div>
              <label className="text-xs text-[var(--color-text-secondary)]">层面</label>
              <Select fullWidth selectSize="sm" value={horizon} onChange={(e) => setHorizon(e.target.value)}>
                {["H1", "H2", "H3"].map((h) => <option key={h}>{h}</option>)}
              </Select>
            </div>
            <div>
              <label className="text-xs text-[var(--color-text-secondary)]">门径阶段</label>
              <Select fullWidth selectSize="sm" value={stageGate} onChange={(e) => setStageGate(e.target.value)}>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              ...
              <Input fullWidth inputSize="sm" type="number" value={nextCommit} onChange={(e) => setNextCommit(e.target.value)} placeholder="实物期权:只押下一笔" />
            </div>
            <div>
              <label className="text-xs text-[var(--color-text-secondary)]">WTP 因子(0–1)</label>
              <Input fullWidth inputSize="sm" type="number" step="0.05" min="0" max="1" value={wtpFactor} onChange={(e) => setWtpFactor(Number(e.target.value))} />
            </div>
            ...
            <div className="mt-2 grid grid-cols-3 gap-2">
              {([
                ...
                <div key={k}>
                  ...
                  <Input fullWidth inputSize="sm" type="number" step="any" value={econ[k]} onChange={(e) => setEcon((c) => ({ ...c, [k]: e.target.value }))} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className={sectionCls}>
            ...
            {dims.map((d, i) => (
              <div key={i} className="mt-2 flex items-center gap-2">
                <Input fullWidth inputSize="sm" placeholder="key(如 material/mrl/cost)" value={d.key} onChange={(e) => setDims((c) => c.map((x, j) => j === i ? { ...x, key: e.target.value } : x))} />
                <Input inputSize="sm" className="w-20" type="number" min="0" max="100" title="得分" value={d.score} onChange={(e) => setDims((c) => c.map((x, j) => j === i ? { ...x, score: Number(e.target.value) } : x))} />
                <Select selectSize="sm" wrapperClassName="w-20" title="证据级" value={d.evidenceLevel} onChange={(e) => setDims((c) => c.map((x, j) => j === i ? { ...x, evidenceLevel: Number(e.target.value) } : x))}>
          ...
          <div className={sectionCls}>
            ...
            {odi.map((o, i) => (
              <div key={i} className="mt-2 flex items-center gap-2">
                ...
                <Input inputSize="sm" className="w-20" type="number" min="0" max="10" value={o.importance} onChange={(e) => setOdi((c) => c.map((x, j) => j === i ? { ...x, importance: Number(e.target.value) } : x))} />
                <label className="text-caption">满意度</label>
                <Input inputSize="sm" className="w-20" type="number" min="0" max="10" value={o.satisfaction} onChange={(e) => setOdi((c) => c.map((x, j) => j === i ? { ...x, satisfaction: Number(e.target.value) } : x))} />
          ...
          <div className={sectionCls}>
            ...
            {gaps.map((g, i) => (
              <div key={i} className="mt-2 grid grid-cols-[1fr_70px_70px_70px_40px] items-center gap-2">
                <Input fullWidth inputSize="sm" placeholder="能力" value={g.capability} onChange={(e) => setGaps((c) => c.map((x, j) => j === i ? { ...x, capability: e.target.value } : x))} />
                <Input fullWidth inputSize="sm" type="number" step="0.1" min="0" max="1" title="内部就绪 0-1" value={g.internalReadiness} onChange={(e) => setGaps((c) => c.map((x, j) => j === i ? { ...x, internalReadiness: Number(e.target.value) } : x))} />
                <Input fullWidth inputSize="sm" type="number" title="时间窗(月)" value={g.windowMonths} onChange={(e) => setGaps((c) => c.map((x, j) => j === i ? { ...x, windowMonths: Number(e.target.value) } : x))} />
                <Input fullWidth inputSize="sm" type="number" title="自研需时(月)" value={g.buildMonths} onChange={(e) => setGaps((c) => c.map((x, j) => j === i ? { ...x, buildMonths: Number(e.target.value) } : x))} />
            ...
            {claims.map((c, i) => (
              <div key={i} className="mt-2 rounded-md border border-[var(--surface-border)] p-2">
                <div className="flex items-center gap-2">
                  <Select selectSize="sm" wrapperClassName="w-16" value={c.axis} onChange={(e) => setClaims((cur) => cur.map((x, j) => j === i ? { ...x, axis: e.target.value } : x))}>
                    <option value="d">D</option><option value="f">F</option><option value="v">V</option>
                  </Select>
                  <Input fullWidth inputSize="sm" placeholder="论点 Claim" value={c.claim} onChange={(e) => setClaims((cur) => cur.map((x, j) => j === i ? { ...x, claim: e.target.value } : x))} />
                  <button type="button" className={delBtn} onClick={() => setClaims((cur) => cur.filter((_, j) => j !== i))}>删</button>
                </div>
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <Input fullWidth inputSize="sm" placeholder="推理 Warrant" value={c.warrant} onChange={(e) => setClaims((cur) => cur.map((x, j) => j === i ? { ...x, warrant: e.target.value } : x))} />
                  <Input fullWidth inputSize="sm" placeholder="反证 Rebuttal(什么条件下不成立)" value={c.rebuttal} onChange={(e) => setClaims((cur) => cur.map((x, j) => j === i ? { ...x, rebuttal: e.target.value } : x))} />
                </div>
                <div className="mt-2">
                  ...
                  {c.evidence.map((e, k) => (
                    <div key={k} className="mt-1 flex items-center gap-2">
                      <Select selectSize="sm" wrapperClassName="w-20" value={e.level} onChange={(ev) => setClaims((cur) => cur.map((x, j) => j === i ? { ...x, evidence: x.evidence.map((y, m) => m === k ? { ...y, level: Number(ev.target.value) } : y) } : x))}>
                        {[1, 2, 3, 4, 5, 6].map((l) => <option key={l} value={l}>L{l}</option>)}
                      </Select>
                      <Input fullWidth inputSize="sm" placeholder="来源/引用(必须可溯源)" value={e.source} onChange={(ev) => setClaims((cur) => cur.map((x, j) => j === i ? { ...x, evidence: x.evidence.map((y, m) => m === k ? { ...y, source: ev.target.value } : y) } : x))} />
                      <Input
                        fullWidth
                        inputSize="sm"
                        className={e.level >= 3 && !e.artifactRef.trim() ? "!border-[var(--signal-yellow)]" : undefined}
          ...
          <div className={sectionCls}>
            ...
            {assumptions.map((a, i) => (
              <div key={i} className="mt-2 grid grid-cols-[90px_1fr_100px_40px] items-center gap-2">
                <Input fullWidth inputSize="sm" placeholder="编号" value={a.code} onChange={(e) => setAssumptions((c) => c.map((x, j) => j === i ? { ...x, code: e.target.value } : x))} />
                <Input fullWidth inputSize="sm" placeholder="假设陈述" value={a.statement} onChange={(e) => setAssumptions((c) => c.map((x, j) => j === i ? { ...x, statement: e.target.value } : x))} />
                <Select fullWidth selectSize="sm" value={a.status} onChange={(e) => setAssumptions((c) => c.map((x, j) => j === i ? { ...x, status: e.target.value } : x))}>

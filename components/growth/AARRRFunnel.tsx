  ...
  return (
    <section className="rounded-lg border border-[var(--stack-gtm)]/30 bg-[var(--color-bg-surface)] p-6">
      ...
      <p className="mb-4 mt-1 text-[var(--type-label)] leading-relaxed text-[var(--color-text-muted)]">
        ...
        <span className="ml-1 text-[var(--signal-red-text)]">红色 = 转化低于基准，即漏点</span>。
      </p>
        ...
        {stages.map((s, i) => {
          ...
          return (
            <div key={s.stage}>
              ...
              />
              {s.leakNote && (
                <p className="mt-1 text-[var(--type-label)] text-[var(--signal-red-text)]">⚠ {s.leakNote}</p>
              )}
            </div>
          );
        })}
      ...
      <p className="mt-4 border-t border-[var(--surface-border)] pt-3 text-xs text-[var(--color-text-secondary)]">
        {leaks.length > 0 ? (
          <>结论：漏点在<span className="font-medium text-[var(--signal-red-text)]">{leaks.map((s) => `「${s.label}」`).join("、")}</span>，优先把资源投到这一段，而非继续加大获客。</>
        ) : (

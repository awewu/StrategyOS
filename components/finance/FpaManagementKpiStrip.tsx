...
export function FpaManagementKpiStrip({ kpis }: { kpis: ManagementKpis }) {
  ...
  return (
    <section className="rounded-lg border border-[var(--color-accent)]/25 bg-[var(--color-bg-surface)] p-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-sm font-medium text-[var(--color-accent)]">FPA 管理报表 · {kpis.period}</h2>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {items.map((item) => (
          <div key={item.label} className="rounded border border-[var(--surface-border)] bg-[var(--surface-raised)] p-4">
            <div className="text-xs text-[var(--color-text-muted)]">{item.label}</div>
            <div className="mt-1 font-data text-xl text-[var(--color-accent)]">{item.value}</div>
